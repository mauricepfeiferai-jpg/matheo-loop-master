#!/usr/bin/env python3
"""Workflow Engine — chained workflows, nicht nur einzelne Tasks.
...
def example_workflow():
    """Demo: Health-Check Workflow"""
    wf = Workflow("health_check")
    wf.add(WorkflowStep("check_disk", "Disk Check", lambda ctx: {"disk_pct": 79.2}))
    wf.add(WorkflowStep("check_memory", "Memory Check", lambda ctx: {"mem_pct": 45.0}))
    wf.add(WorkflowStep("check_sensors", "Sensor Check", lambda ctx: {"sensors_alert": True}, depends_on=["check_disk", "check_memory"]))
    return wf
