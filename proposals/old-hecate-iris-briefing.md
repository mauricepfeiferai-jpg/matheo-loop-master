...
loop: old-hecate-iris-briefing
erstellt: durch hecate.loop_factory
---

# Loop-Proposal: old-hecate-iris-briefing

**Zweck:** Iris Daily Briefing aus altem HECATE v1 beibehalten
**Schedule:** `30 7 * * *`
...
```bash
#!/bin/bash
# old-hecate-iris-briefing — Ledger-instrumentiert ab Geburt (kein Beweis = kein Erfolg)
LEDGER="python3 -m hecate.ledger"
cd /root/projects/loop-master
RID=$($LEDGER start old-hecate-iris-briefing)
OUTPUT=$(mktemp /var/lib/loop-master/old-hecate-iris-briefing.XXXX.out)
if bash /root/hecate/iris/iris_briefing.sh > "$OUTPUT" 2>&1; then
...
1. Ein echter Lauf erzeugt einen ok-Eintrag: `python3 -m hecate.ledger report --loop old-hecate-iris-briefing`
