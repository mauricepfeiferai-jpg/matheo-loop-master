...
loop: housekeep-deprecated-openclaw
erstellt: durch hecate.loop_factory
---

# Loop-Proposal: housekeep-deprecated-openclaw

**Zweck:** Housekeeping: archive_to_backup /root/openclaw (3.65 GB)
**Schedule:** `einmalig`
...
```bash
#!/bin/bash
# housekeep-deprecated-openclaw — Ledger-instrumentiert ab Geburt (kein Beweis = kein Erfolg)
LEDGER="python3 -m hecate.ledger"
cd /root/projects/loop-master
RID=$($LEDGER start housekeep-deprecated-openclaw)
OUTPUT=$(mktemp /var/lib/loop-master/housekeep-deprecated-openclaw.XXXX.out)
if python3 -m hecate.system_housekeeper apply openclaw > "$OUTPUT" 2>&1; then
...
1. Ein echter Lauf erzeugt einen ok-Eintrag: `python3 -m hecate.ledger report --loop housekeep-deprecated-openclaw`
